...
for i=1:m
    ...
    costFunc = @(p) sparseAutoencoderCost(p,visibleSize,hiddenSize,lambda, sparsityParam, beta,X_train(:,1:m));
    options = optimset('MaxIter', 600);
    [opttheta,~] = fmincg(costFunc,theta,options);
    [error_train(i),~] = sparseAutoencoderCost(opttheta,visibleSize,hiddenSize,0,sparsityParam,0,X_train(:,1:i));
    [error_val(i),~] = sparseAutoencoderCost(opttheta,visibleSize,hiddenSize,0,sparsityParam,0,X_val);
end
